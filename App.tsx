...
import { loadWordsFromFile, saveWordsToFile } from './utils/fileStorage';
                         ...
                         <div className="flex flex-col gap-2 border-b pb-4">
                            ...
                            <div className="flex gap-2">
                                <input 
                                    ...
                                    className="border rounded px-2 flex-1 h-8 text-sm" 
                                    placeholder="AI-nyckel..." 
                                />
                                <button onClick={saveApiKey} className="bg-blue-500 text-white px-2 h-8 rounded text-xs font-bold">Spara</button>
                            </div>
                         </div>
                         <div className="flex gap-2">
                            <input 
...
const App: React.FC = () => {
  const [allWords, setAllWords] = useState<AnalyzedWord[]>([]);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      if (!process.env.API_KEY && !manualKey) {
        setApiKeyMissing(true);
      } else {
        setApiKeyMissing(false);
      }

      // Load words from file (same approach as audio export/import)
      try {
        console.log('App: Loading words from file...');
        const storedWords = await loadWordsFromFile();
        console.log(`App: Loaded ${storedWords.length} words from file`);
        if (storedWords.length > 0) {
          setAllWords(storedWords);
          setCurrentWordId(storedWords[0].id);
          console.log(`App: Set current word ID to ${storedWords[0].id}`);
        } else {
          console.log('App: No words in file, starting with empty list');
        }
        // If no words in file, leave empty - user can add words later
      } catch (error) {
        console.error('App: Failed to load words from file:', error);
        // Don't set any words on error
      }
  ...
  // Save words to file whenever they change (auto-save)
  useEffect(() => {
    const saveWords = async () => {
      try {
        console.log(`App: Auto-saving ${allWords.length} words to file...`);
        await saveWordsToFile(allWords);
        console.log(`App: Successfully auto-saved ${allWords.length} words to file`);
      } catch (error) {
        console.error('App: Failed to auto-save words to file:', error);
      }
    };

    // Save words (even if empty - to clear file when all words are deleted)
    saveWords();
  }, [allWords]);
  ...
  const handleGenerate = useCallback(async () => {
    const manualKey = localStorage.getItem('GEMINI_API_KEY');
    if (!process.env.API_KEY && !manualKey) { alert("API Key missing. Add it in Expert settings."); return; }
