...
interface AvatarProps {
  isTalking: boolean;
  visemeId?: number | null; // ID of the specific mouth shape (0-49)
  className?: string;
}

export const Avatar: React.FC<AvatarProps> = ({ isTalking, visemeId, className = "w-24 h-24" }) => {
  const [mouthOpen, setMouthOpen] = useState(0); // 0 = closed, 1 = open
  const isTalkingRef = useRef(isTalking);

  // Sync ref with prop for use inside the loop
  useEffect(() => {
      isTalkingRef.current = isTalking;
  }, [isTalking]);

  useEffect(() => {
    // Connect to the audio analyzer
    // We only register once, and check the ref inside the callback
    const unsubscribe = registerAmplitudeListener((level) => {
      ...
      if (speaking) {
        if (isPlayingAudioContext()) {
            // Real recorded audio is playing: Trust the analyzer (Lip Sync)
            // Amplify level slightly for better visibility
            const openAmount = Math.min(1, level * 5); 
            setMouthOpen(level > SILENCE_THRESHOLD ? openAmount : 0);
        } else {
            // TTS (SpeechSynthesis) or unknown source: Analyzer is blind (Level 0)
            // Fallback: Simulate natural mouth movement
            const time = Date.now();
            const wave = (Math.sin(time / 80) + 1) / 2; // Rhythmic 0-1
            const flutter = Math.random() * 0.2; // Add organic randomness
            
            // Keep mouth somewhat open when talking
  ...
  // Use explicit viseme if provided, otherwise fallback to calculated amplitude/simulation
  let mouthPath = "";
  let showTeeth = false;

  if (visemeId !== undefined && visemeId !== null && VISEMES[visemeId]) {
      // Use configured mouth
      mouthPath = VISEMES[visemeId];
  } else {
      // Use auto-generated mouth based on 'mouthOpen' state
      const mouthCenterY = 82;
      const maxOpenHeight = 12;
      const currentHeight = mouthOpen * maxOpenHeight;
      
      mouthPath = mouthOpen < 0.1 
        ? `M 40 ${mouthCenterY} Q 50 ${mouthCenterY + 2} 60 ${mouthCenterY}` // Closed smile
        : `M 40 ${mouthCenterY} 
           Q 50 ${mouthCenterY - currentHeight/2} 60 ${mouthCenterY} 
           Q 50 ${mouthCenterY + currentHeight * 1.5} 40 ${mouthCenterY}`; // Open oval
  ...
  return (
    <div className={`${className} relative`}>
      <svg viewBox="0 0 100 110" className="w-full h-full drop-shadow-xl">
        ...
        {/* Head Shape - Expanded top to fix hair gap */}
        <path 
            d="M 15 5 
               L 85 5 
               L 88 50 
        ...
        {/* Eyes */}
        <g>
            <circle cx="35" cy="55" r="5" fill="#FFF" />
        ...
        {/* Teeth hint (only for auto-mode, static visemes include teeth in path if needed or simplify) */}
        {showTeeth && !visemeId && (
           <path d="M 42 82 Q 50 84 58 82" stroke="#FFF" strokeWidth="1.5" fill="none" opacity="0.8" />
        )}
      </svg>
    </div>
  );
