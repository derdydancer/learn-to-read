...
export const VOWELS: SoundItem[] = [
  ...
  { id: 'v_o_short', label: 'O (Kort)', char: 'o', variant: 'short', example: 'Ostadig, Boll' }, // 'o' is tricky, sometimes 'å' sound
  // U
...
export const ALL_SOUNDS = [...VOWELS, ...CONSONANTS, ...SPECIALS];
...
export const getSoundListForPrompt = (): string => {
    return ALL_SOUNDS.map(s => `- ID: "${s.id}" | Ljud: ${s.label} | Exempel: ${s.example}`).join('\n');
};